...
with testing as (
select *, 
       count(*) over (partition by user_id) as tot, 
       count(*) over (partition by user_id, reaction) as sub
from reactions
)


select distinct user_id, reaction as dominant_reaction, round(sub/tot, 2) as reaction_ratio
from testing
where tot >= 5 and round(sub/tot, 2) >= 0.6
