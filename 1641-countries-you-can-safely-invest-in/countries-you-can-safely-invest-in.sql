...
testing2 as (
select t.country, ca.duration
from testing as t
join calls as ca
on t.pid = ca.caller_id
...
from testing as t
join calls as ca
on t.pid = ca.callee_id
)

select country
from testing2
group by country
having (sum(duration) / count(country)) > (select ((2 * sum(duration)) / (2 * count(duration))) as global_durt from calls)
