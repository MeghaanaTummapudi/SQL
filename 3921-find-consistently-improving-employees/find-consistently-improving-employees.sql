...
with testing as (
select t.*, rank() over (partition by t.employee_id order by t.review_date desc) as rn
...
where (t.n_c >= 3 and t.l3 is not null and t.l2 is not null and t.l1 is not null) 
)
...
select t.employee_id, e.name, t.l3 - t.l1 as improvement_score
from testing as t
left join employees as e
on t.employee_id = e.employee_id
where t.rn = 1 and t.l3 > t.l2 and t.l2 > t.l1
